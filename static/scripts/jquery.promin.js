    ...
    const pmethods = {
        init(opt) {
            ...
            $steps.hide().each(function(i, e) {
                const $e = $(e);
                const field = pmethods.getField($e);
        ...
        resetSelect(i, e) {
            const $e = $(e);
            const val = $e.find('option[selected]').attr('value');
            $e.val(val);
        },

        getField(e) {
            const tag = e.prop('nodeName').toLowerCase();
            const desc = e.find('input, textarea, select');
